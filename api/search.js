...
function getKnowledgeBase() {
  if (!knowledgeBase) {
    // Generated by scripts/build-knowledge-base.js during `npm run build`.
    knowledgeBase = require('../static/knowledge-base.json');
  }
...
module.exports = function handler(req, res) {
  ...
  try {
    const searchResult = searchDocuments(query, getKnowledgeBase().documents, {
      channels: body.channels,
      types: body.types,
      limit,
    });
    // searchDocuments is synchronous; tookMs is for latency logging only.
    // Platform (Vercel) enforces the hard request timeout.
