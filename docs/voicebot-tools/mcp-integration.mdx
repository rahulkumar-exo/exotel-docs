...
:::warning This is a session cookie, not an API key
`access_token` expires when you log out of `my.exotel.com`. Anyone who has it can act as you against the Tools Server until it expires. Do not commit it, paste it into tickets, or share it. The VoiceBot REST APIs use separate API keys (see [Overview → VoiceBot](/docs/mcp-server/overview#voicebot)). The Tools Server still authenticates with this cookie.
:::
