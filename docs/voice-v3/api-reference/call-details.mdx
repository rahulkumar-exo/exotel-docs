...
sidebar_label: Call Details (CCM)
...
<TryItBanner method="GET" endpoint="/v3/accounts/{account_sid}/calls/{call_sid}" />

:::info
This is a **CCM** (Contact Center Management) API. Use it for calls placed with [Connect Agent to Customer](/docs/voice-v3/api-reference/make-a-call) if you run contact center operations on `my*.exotel.com`. For calls that connect two phone numbers, see [Call Details](/docs/voice-v1/api-reference/call-details).
:::

<RegionalUrls path="/v3/accounts/{account_sid}/calls/{call_sid}" host="ccm" />
...
| `call_sid` | String | Unique call identifier from the [Connect Agent to Customer](/docs/voice-v3/api-reference/make-a-call) response |
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_URL => "https://ccm-api.exotel.com/v3/accounts/<account_sid>/calls/<call_sid>",
...
func main() {
    url := "https://ccm-api.exotel.com/v3/accounts/<account_sid>/calls/<call_sid>"
...
<ApiConsole
  ...
  host="ccm"
  params={[
    { name: 'call_sid', required: true, type: 'string', description: 'Unique call identifier from the Connect Agent to Customer response', placeholder: 'cabfb1dfa8c5e1c7f2c458423b7716b3' },
  ]}
/>
