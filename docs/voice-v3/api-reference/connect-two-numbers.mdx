...
Use the **Voice v1** endpoint (`/v1/Accounts/...`). After the call, use [Call Details (CCM)](/docs/voice-v3/api-reference/call-details) for the enriched record.
:::
...
curl_setopt_array($curl, [
  CURLOPT_URL            => "https://api.exotel.com/v1/Accounts/{$accountSid}/Calls/connect",
...
|-------|------|-------------|
| `Call.Sid` | String | Unique call identifier. Use this with [Call Details (CCM)](/docs/voice-v3/api-reference/call-details) for the enriched record. |
...
| `429` | Rate limit exceeded (20 calls/minute). |
| `500` | Server error. |
...
The enriched Call Details response includes DTMF digits, recording objects, app/flow information, and per-leg data. See [Call Details (CCM)](/docs/voice-v3/api-reference/call-details).
...
<ApiConsole
  ...
  params={[
    ...
    { name: 'TimeLimit', required: false, type: 'number', description: 'Max call duration in seconds (max 14400)', placeholder: '600' },
