...
curl -u "$EXOTEL_API_KEY:$EXOTEL_API_TOKEN" -X POST \
  "https://$EXOTEL_SUBDOMAIN/v1/Accounts/$EXOTEL_ACCOUNT_SID/Sms/send" \
  -d "From=EXOTEL" \
  -d "To=+919876543210" \
...
$ch = curl_init("https://{$subdomain}/v1/Accounts/{$accountSid}/Sms/send");
...
func main() {
    ...
    endpoint := fmt.Sprintf("https://%s/v1/Accounts/%s/Sms/send", subdomain, accountSid)
    data := url.Values{}
    data.Set("From", "EXOTEL")
    data.Set("To", "+919876543210")
    data.Set("Body", "Hello from Exotel! Your OTP is 123456")
    data.Set("DltEntityId", "YOUR_DLT_ENTITY_ID")
    data.Set("DltTemplateId", "YOUR_DLT_TEMPLATE_ID")
...
curl -u "$EXOTEL_API_KEY:$EXOTEL_API_TOKEN" \
  "https://$EXOTEL_SUBDOMAIN/v1/Accounts/$EXOTEL_ACCOUNT_SID/SMS/Messages/YOUR_SMS_SID.json"
```
