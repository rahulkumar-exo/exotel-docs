...
<TryItBanner method="GET" endpoint="/v2/accounts/{sid}/calls/{call_sid}" />
...
<ApiConsole
  ...
  host="ccm"
  params={[
    { name: 'call_sid', required: true, type: 'string', description: 'The unique call identifier from the Make Call response', placeholder: 'unique_call_identifier' },
  ]}
/>
