...
id: login
title: Login
description: Authenticate and create a session for Exotel Contact Center v4 API access. Covers login endpoints, request headers, and base URLs by data center.
sidebar_label: Login
---
...
<TryItBanner method="POST" endpoint="/v4/accounts/{sid}/login" />
...
<ApiConsole
  method="POST"
  path="/v4/accounts/{account_sid}/login"
  contentType="json"
  params={[
    { name: 'username', required: true, type: 'string', description: 'API key from Exotel Dashboard', placeholder: 'your_api_key' },
    { name: 'password', required: true, type: 'string', description: 'API token from Exotel Dashboard', placeholder: 'your_api_token' },
    { name: 'account_sid', required: true, type: 'string', description: 'Your Exotel account SID', placeholder: 'your_account_sid' },
  ]}
/>
